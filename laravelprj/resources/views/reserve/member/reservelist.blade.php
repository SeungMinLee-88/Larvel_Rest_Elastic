                                ...
                                <ul class="list-group">
                                    <li class="list-group-item" style="padding: 0rem 0rem;padding-top: 0rem;padding-right: 0rem;padding-bottom: 0rem;padding-left: 0rem;">
                                        ...
                                        <div class="pull-right action-buttons" style="margin-left: 700px">
                                            <a href="{{ route('member.reserveedit',["reserveid" => $reserve->id, "userid" => $reserve->user->id, "username" => $reserve->user->name, "hallid" => $reserve->hall->id, "hallname" => $reserve->hall->hallname, "reservedate" => $reserve->reserve_date, "reservereason" => $reserve->reserve_reason, "month" => date("m",strtotime($reserve->reserve_date))])}}"><span class="glyphicon glyphicon-pencil"></span></a>
...
function ajaxfunction(reserveid){
            ...
            $(document).ready(function () {
                alert("/member/reservedelete?reserveid="+reserveid);
                $.ajax({
                    type : "GET",
                    url : "/member/reservedelete?reserveid="+reserveid,
                    dataType : "json",
                    error : function(){
                        alert("통신실패!!!!");
                    },
